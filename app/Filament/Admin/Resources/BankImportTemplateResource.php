            ...
            Tabs::make()->tabs([
                ...
                Tab::make(__('General'))->schema([
                    ...
                    Forms\Components\TextInput::make('name')
                        ...
                        ->required()
                        ->maxLength(100)
                        ->placeholder(__('e.g. Al-Rajhi statement — current account')),
                    Forms\Components\Toggle::make('is_default')
                        ->label(__('Default for this bank'))
                        ->helperText(__('When members import CSV for this bank, this template is selected first. Only one default per bank.')),
                ])->columns(2),

                Tab::make(__('File & CSV'))->schema([
                    Section::make(__('Separator & character set'))
                        ->description(__('Must match how the bank exports the file.'))
                        ->compact()
                        ->schema([
                    ...
                    Section::make(__('Layout'))
                        ...
                        ->schema([
                            ...
                            Forms\Components\TextInput::make('skip_rows')
                                ...
                                ->default(0)
                                ->minValue(0)
                                ->helperText(__('Use for title rows, logos, or metadata above the table.')),
                        ])->columns(2),
                ]),
                ...
                Tab::make(__('Column mapping'))->schema([
                    Forms\Components\Placeholder::make('column_mapping_intro')
                        ...
                        ->content(new HtmlString(
                            ...
                            .__('Map each CSV column to a field. Amounts may include a currency code or symbol with spaces (e.g. :example) — the importer strips that automatically.', [
                                'example' => '<code class="text-xs rounded bg-gray-100 px-1 py-0.5 dark:bg-white/10">SAR 1,234.50</code>',
                            ])
                            .'</p>'
                        ))
                        ->columnSpanFull(),
                    ...
                    Section::make(__('Transaction date'))
                        ->description(__('Which column holds the booking date, and how it is formatted.'))
                        ->compact()
                        ->schema([
                            Forms\Components\TextInput::make('date_column')
                                ...
                                ->required()
                                ->placeholder(__('Date'))
                                ->helperText(__('Header name, or column index if the file has no header.')),
                            Forms\Components\TextInput::make('date_format')
                                ->label(__('PHP date format'))
                                ->required()
                                ->default('Y-m-d')
                                ->placeholder(__('d/m/Y'))
                                ->helperText(__('Examples: d/m/Y, m/d/Y, Y-m-d, d-M-Y')),
                        ])->columns(2),

                    Section::make(__('Amount'))
                        ->description(__('How debit/credit amounts appear in the file.'))
                        ->compact()
                        ->schema([
                            ...
                            Forms\Components\TextInput::make('amount_column')
                                ...
                                ->helperText(__('Values may include currency text or spaces before the number.')),
                            Forms\Components\TextInput::make('credit_column')
                                ...
                                ->helperText(__('Column for incoming / credit amounts.')),
                            Forms\Components\TextInput::make('debit_column')
                                ->label(__('Debit column'))
                                ->visible(fn ($get) => $get('amount_type') === 'split')
                                ->required(fn ($get) => $get('amount_type') === 'split')
                                ->helperText(__('Column for outgoing / debit amounts. Negative values are treated as debits using the absolute amount.')),
                        ])->columns(2),

                    Section::make(__('Optional CSV columns'))
                        ->description(__('Map extra columns by key. Use keys reference, description, or balance to fill the main transaction fields (same as bank columns). Any other key (e.g. branch_code) is stored only on the import row. Header name or 0-based index.'))
                        ->schema([
                            Forms\Components\Repeater::make('optional_columns')
                                ->label(__('Fields'))
                                ->helperText(__('Example: key reference → CSV column "Reference No."; key description → "Narration"; key balance → "Balance".'))
                                ->live()
                                ->schema([
                                    Forms\Components\TextInput::make('key')
                                        ...
                                        ->helperText(__('Use reference, description, or balance for standard fields, or any label for extras.')),
                                    Forms\Components\TextInput::make('column')
                                        ->label(__('CSV column'))
                                        ->required()
                                        ->maxLength(100)
                                        ->placeholder(__('Reference or 4'))
                                        ->helperText(__('Header name or 0-based index.')),
                                ])
                                ->defaultItems(0)
                                ->columns(2)
                                ->reorderable()
                ...
                Tab::make(__('Duplicate detection'))->schema([
                    Section::make(__('Matching rules'))
                        ->description(__('A row is treated as a duplicate only if every selected field matches an earlier transaction. Add optional column keys below when you need extra fields in the match.'))
                        ->compact()
                        ->schema([
                            Forms\Components\CheckboxList::make('duplicate_match_fields')
                                ...
                                ->default(['date', 'amount'])
                                ->columns(2)
                                ->helperText(__('Reference and Description appear here only when you map those keys under Optional CSV columns. If nothing valid is selected, the system uses date + amount (+ reference when mapped).')),
                            Forms\Components\TextInput::make('duplicate_date_tolerance')
                                ...
                                ->minValue(0)
                                ->maxValue(30)
                                ->helperText(__('0 = same calendar day (within the date column’s precision). Higher values allow nearby dates to match.')),
                        ]),
                ]),
