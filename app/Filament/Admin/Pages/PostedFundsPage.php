...
class PostedFundsPage extends Page implements HasTable
{
    use InteractsWithTable;

    protected string $view = 'filament.admin.pages.posted-funds-page';

    protected static bool $shouldRegisterNavigation = false;

    protected static ?string $slug = 'posted-funds';

    public Account $account;

    public function mount(): void
    {
        $accountId = (int) request()->integer('account');

        abort_unless($accountId > 0, 404);
        ...
        abort_unless(auth()->user()?->can('view', $this->account), 403);
    }
    ...
    public function getHeaderActions(): array
    {
        return [
            Action::make('backToLedger')
                ...
                ->color('gray')
                ->url(AccountResource::getUrl('view', ['record' => $this->account])),
        ];
    }
