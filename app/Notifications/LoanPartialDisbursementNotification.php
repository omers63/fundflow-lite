...
class LoanPartialDisbursementNotification extends Notification
{
    ...
    public function toMail(mixed $notifiable): MailMessage
    {
        ...
        $vars = ['name' => $notifiable->name, 'amount' => $portion, 'total' => $total, 'approved' => $approved, 'remaining' => $remaining];
        $subject = EmailTemplateService::render(
            EmailTemplateService::get('loan_partial_disbursement', 'subject', $locale, $this->tr('FundFlow — Loan Partially Disbursed', 'FundFlow — تم صرف القرض جزئيًا')),
            $vars
        );
        $greeting = EmailTemplateService::render(
            EmailTemplateService::get('loan_partial_disbursement', 'greeting', $locale, $this->tr('Dear :name,', 'عزيزي/عزيزتي :name،', ['name' => $notifiable->name])),
            $vars
        );
        $bodyLines = EmailTemplateService::renderLines(
            EmailTemplateService::get('loan_partial_disbursement', 'body', $locale, implode("\n", [
                $this->tr('A partial disbursement of **:amount** has been credited against your approved loan.', 'تم قيد دفعة جزئية بمبلغ **:amount** على قرضك المعتمد.', ['amount' => $portion]),
                $this->tr('**Total disbursed so far:** :total of :approved', '**إجمالي المصروف حتى الآن:** :total من :approved', ['total' => $total, 'approved' => $approved]),
                $this->tr('**Remaining to disburse:** :remaining', '**المتبقي للصرف:** :remaining', ['remaining' => $remaining]),
                $this->tr('Repayment will begin only after the loan is fully disbursed.', 'سيبدأ السداد فقط بعد صرف القرض بالكامل.'),
            ])),
            $vars
        );
        $actionLabel = EmailTemplateService::render(
            EmailTemplateService::get('loan_partial_disbursement', 'action_label', $locale, $this->tr('View My Loans', 'عرض قروضي')),
            $vars
        );
