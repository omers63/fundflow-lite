...
<div class="body">
        ...
        <div class="two-col">
            <div class="col">
                <table class="info-table">
                    <tr><td class="lbl">{{ __('Full Name') }}</td><td class="val">{{ $m['name'] ?? $statement->member->user->name }}</td></tr>
                    <tr><td class="lbl">{{ __('Member No.') }}</td><td class="val">{{ $m['member_number'] ?? $statement->member->member_number }}</td></tr>
                    <tr><td class="lbl">{{ __('Email') }}</td><td class="val">{{ $m['email'] ?? $statement->member->user->email }}</td></tr>
                    <tr><td class="lbl">{{ __('Phone') }}</td><td class="val" dir="ltr" style="unicode-bidi:isolate;">{{ \App\Support\PhoneDisplay::plain($m['phone'] ?? $statement->member->user->phone ?? null) }}</td></tr>
                    <tr><td class="lbl">{{ __('Member Since') }}</td><td class="val">{{ isset($m['joined_at']) ? \Carbon\Carbon::parse($m['joined_at'])->format('d M Y') : ($statement->member->joined_at?->format('d M Y') ?? '—') }}</td></tr>
                </table>
            </div>
            <div class="col">
                <table class="info-table">
                    <tr><td class="lbl">{{ __('Cash Balance') }}</td><td class="val" style="color:{{ $accent }};">{{ __('SAR') }} {{ $cashClose }}</td></tr>
                    <tr><td class="lbl">{{ __('Fund Balance') }}</td><td class="val" style="color:#6d28d9;">{{ __('SAR') }} {{ $fundClose }}</td></tr>
                    <tr><td class="lbl">{{ __('Monthly Contribution') }}</td><td class="val">{{ __('SAR') }} {{ number_format($m['monthly_contrib'] ?? $statement->member->monthly_contribution_amount) }}</td></tr>
                    <tr><td class="lbl">{{ __('Cash (Opening)') }}</td><td class="val">{{ __('SAR') }} {{ $cashOpen }}</td></tr>
                    <tr><td class="lbl">{{ __('Fund (Opening)') }}</td><td class="val">{{ __('SAR') }} {{ $fundOpen }}</td></tr>
                </table>
            </div>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <table class="summary">
            ...
            <tr class="highlight">
                <td>+ {{ __('Contribution') }} ({{ $c['paid_at'] ?? __('this period') }}) {{ $c['is_late'] ? '<span class="badge-overdue">'.__('LATE').'</span>' : '' }}</td>
            ...
            @foreach($insts as $i)
            <tr>
                <td>− {{ __('Loan Repayment') }} #{{ $i['installment_number'] }} ({{ __('due') }} {{ $i['due_date'] }}, {{ __('paid') }} {{ $i['paid_at'] }}){{ (float)$i['late_fee'] > 0 ? ' + '.__('late fee').' SAR '.number_format($i['late_fee'], 2) : '' }}</td>
    ...
    <div class="section">
        ...
        <div class="two-col">
            <div class="col">
                <table class="info-table">
                    <tr><td class="lbl">{{ __('Loan ID') }}</td><td class="val">#{{ $loan['id'] }}</td></tr>
                    <tr><td class="lbl">{{ __('Status') }}</td><td class="val">{{ ucfirst($loan['status']) }}</td></tr>
                    <tr><td class="lbl">{{ __('Approved Amount') }}</td><td class="val">{{ __('SAR') }} {{ number_format($loan['amount_approved'], 2) }}</td></tr>
                    <tr><td class="lbl">{{ __('Remaining') }}</td><td class="val" style="color:#dc2626;">{{ __('SAR') }} {{ number_format($loan['remaining_amount'], 2) }}</td></tr>
                    <tr><td class="lbl">{{ __('Tier') }}</td><td class="val">{{ $loan['tier'] ?? '—' }}</td></tr>
                </table>
            </div>
            <div class="col">
                <table class="info-table">
                    <tr><td class="lbl">{{ __('Disbursed') }}</td><td class="val">{{ $loan['disbursed_at'] ?? '—' }}</td></tr>
                    <tr><td class="lbl">{{ __('Installments Paid') }}</td><td class="val">{{ $loanPaid }} / {{ $loanTotal }}</td></tr>
                    <tr><td class="lbl">{{ __('Remaining') }}</td><td class="val">{{ $loan['installments_pending'] ?? 0 }}</td></tr>
                    <tr><td class="lbl">{{ __('Next Due') }}</td><td class="val">{{ $loan['next_due'] ?? '—' }}</td></tr>
                </table>
                <div style="margin-top: 8px;">
                    <div style="font-size:9px; color:#64748b; margin-bottom:2px;">{{ __('Repayment progress') }}: {{ $loanPct }}%</div>
        ...
        <div class="alert" style="margin-top:10px;">
            ...
            @foreach($overdue as $ov)
                {{ __('Installment') }} #{{ $ov['installment_number'] }} {{ __('due') }} {{ $ov['due_date'] }}: {{ __('SAR') }} {{ number_format($ov['amount'], 2) }}
                @if($ov['late_fee'] > 0) + {{ __('SAR') }} {{ number_format($ov['late_fee'], 2) }} {{ __('late fee') }} @endif.
            @endforeach
        </div>
        @endif
        @else
            ...
            <div class="col">
                <div class="score-row">
                    ...
                    <div class="score-cell">
                        ...
                        <div style="font-size:10px; margin-top:4px;">{{ __('Based on :count contribution records', ['count' => $totalCnt]) }}</div>
                    </div>
                </div>
            </div>
            <div class="col">
...
<div class="page-footer">
    <div class="pf-left">
        {{ __('Statement ID') }}: STMT-{{ strtoupper($statement->member->member_number) }}-{{ str_replace('-', '', $statement->period) }}
        &nbsp;|&nbsp;
