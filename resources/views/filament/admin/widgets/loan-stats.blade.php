    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-6">
        ...
        <div
            ...
            <div class="pl-2">
                ...
                <p class="mt-0.5 text-xs text-gray-400">
                    {{ __(':amount outstanding', ['amount' => \App\Support\UiNumber::sar($d['active_amount'])]) }}</p>
            </div>
        </div>
        ...
        <div
            ...
            <div class="pl-2">
                ...
                <p class="mt-0.5 text-xs text-gray-400">
                    {{ __(':amount requested', ['amount' => \App\Support\UiNumber::sar($d['pending_queue_amount'])]) }}
                </p>
            </div>
        </div>
        ...
        <div
            class="relative overflow-hidden rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div
                class="absolute inset-y-0 left-0 w-1 rounded-l-xl {{ $d['overdue_count'] > 0 ? 'bg-red-500' : 'bg-emerald-500' }}">
            </div>
            <div class="pl-2">
                <div class="flex items-center gap-1.5 mb-2">
                    <x-heroicon-o-exclamation-triangle
                        class="w-4 h-4 {{ $d['overdue_count'] > 0 ? 'text-red-500' : 'text-emerald-500' }}" />
                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                        {{ __('Overdue') }}</p>
                </div>
                <p
                    ...
                    {{ $d['overdue_count'] }}</p>
                <p class="mt-0.5 text-xs text-gray-400">
                    {{ __(':amount overdue', ['amount' => \App\Support\UiNumber::sar($d['overdue_amount'])]) }}</p>
            </div>
        </div>
        ...
        <div
            ...
            <div class="pl-2">
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white">
                    {{ \App\Support\UiNumber::sar($d['disbursed_this_month']) }}</p>
        ...
        <div
            ...
            <div class="pl-2">
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white">
                    {{ $d['by_status']['completed']['count'] + $d['by_status']['early_settled']['count'] }}</p>
                <p class="mt-0.5 text-xs text-gray-400">
    ...
    <div
        ...
        <div
            ...
            <h4 class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                {{ __('Portfolio Breakdown by Status') }}</h4>
        </div>
        @php
            $statusMeta = [
        ...
        <div class="px-5 py-4">
            ...
            <div class="flex w-full rounded-full overflow-hidden h-3 mb-4 bg-gray-100 dark:bg-gray-700">
                @foreach($statusMeta as $key => $meta)
                    ...
                    @if($pct > 0)
                        <div class="h-3 {{ $meta['color'] }} transition-all" style="width: {{ $pct }}%"
                            title="{{ $meta['label'] }}: {{ $d['by_status'][$key]['count'] }}"></div>
                    @endif
                @endforeach
            </div>
            {{-- Legend --}}
            <div class="flex flex-wrap gap-x-5 gap-y-2">
                @foreach($statusMeta as $key => $meta)
                    ...
                    <div class="flex items-center gap-1.5">
                        ...
                        <span
                            class="text-xs text-gray-400">({{ $d['by_status'][$key]['count'] }}{{ $pct > 0 ? ' · ' . $pct . '%' : '' }})</span>
                    </div>
                @endforeach
            </div>
        </div>
