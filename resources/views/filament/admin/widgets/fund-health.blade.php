    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
        ...
        <div class="flex items-center gap-2">
            ...
            <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-semibold
                ...
                @if($health)
                    <x-heroicon-o-check-circle class="w-3.5 h-3.5" /> Healthy
                @elseif($warn)
                    <x-heroicon-o-exclamation-triangle class="w-3.5 h-3.5" /> Monitor
                @else
                    <x-heroicon-o-x-circle class="w-3.5 h-3.5" /> Action needed
                @endif
            </span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-3 divide-x divide-gray-100 dark:divide-gray-700 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="p-5">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Master Fund</span>
            </div>
            ...
            <p class="mt-1 text-xs text-gray-400">{{ $d['fund_pct'] }}% of total assets</p>
        </div>
        ...
        <div class="p-5">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Cash on Hand</span>
            </div>
            ...
            <p class="mt-1 text-xs text-gray-400">{{ $d['cash_pct'] }}% of total assets</p>
        </div>
        ...
        <div class="p-5">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Loan Exposure</span>
            </div>
            ...
            <p class="mt-1 text-xs text-gray-400">{{ $d['loan_count'] }} active / approved loans</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 p-5 sm:grid-cols-3">
        ...
        <div class="rounded-xl {{ $cp['bg'] }} ring-1 {{ $cp['ring'] }} p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">Fund Coverage Ratio</span>
            </div>
            <p class="text-2xl font-bold {{ $cp['text'] }}">{{ $d['coverage_label'] }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Fund ÷ loan exposure</p>
            ...
            <p class="mt-1.5 text-xs {{ $cp['text'] }} font-medium">
                @if($d['coverage_status'] === 'success') Strong — fund well-covers exposure
                @elseif($d['coverage_status'] === 'warning') Moderate — monitor closely
                @else Low — exposure exceeds fund
                @endif
            </p>
        </div>
        ...
        <div class="rounded-xl {{ $ip['bg'] }} ring-1 {{ $ip['ring'] }} p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">Contribution Compliance</span>
            </div>
            <p class="text-2xl font-bold {{ $ip['text'] }}">{{ $d['compliance_rate'] }}%</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $d['paid_this_month'] }} of {{ $d['active_count'] }} members paid this month</p>
            <div class="mt-3 h-2 rounded-full bg-gray-200 dark:bg-gray-600">
            ...
            <p class="mt-1.5 text-xs {{ $ip['text'] }} font-medium">
                @if($d['compliance_rate'] >= 90) Excellent compliance
                @elseif($d['compliance_rate'] >= 70) Good — room to improve
                @else Low — follow up needed
                @endif
            </p>
        </div>
        ...
        <div class="rounded-xl {{ $op['bg'] }} ring-1 {{ $op['ring'] }} p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">Overdue Exposure</span>
            </div>
            <p class="text-2xl font-bold {{ $op['text'] }}">﷼ {{ number_format($d['overdue_amount'], 0) }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ $d['overdue_count'] }} installment{{ $d['overdue_count'] !== 1 ? 's' : '' }} overdue</p>
            <div class="mt-3 h-2 rounded-full bg-gray-200 dark:bg-gray-600">
                @php
            ...
            <p class="mt-1.5 text-xs {{ $op['text'] }} font-medium">
                @if($d['overdue_amount'] === 0.0) All installments current
