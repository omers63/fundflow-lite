    ...
    <div
        ...
        <div class="p-5">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span
                    class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{ __('Master Fund') }}</span>
            </div>
            <p class="text-xl font-bold text-gray-900 dark:text-white">
                {{ \App\Support\UiNumber::sar($d['master_fund']) }}</p>
            <div class="mt-2 h-1.5 rounded-full bg-gray-100 dark:bg-gray-700">
                <div class="h-1.5 rounded-full bg-emerald-500 transition-all" style="width: {{ $d['fund_pct'] }}%">
                </div>
            </div>
        ...
        <div class="p-5">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span
                    class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{ __('Cash on Hand') }}</span>
            </div>
            <p class="text-xl font-bold text-gray-900 dark:text-white">
                {{ \App\Support\UiNumber::sar($d['master_cash']) }}</p>
            <div class="mt-2 h-1.5 rounded-full bg-gray-100 dark:bg-gray-700">
        ...
        <div class="p-5">
            <div class="flex items-center gap-2 mb-2">
                ...
                <span
                    class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{{ __('Loan Exposure') }}</span>
            </div>
            <p class="text-xl font-bold text-gray-900 dark:text-white">
                {{ \App\Support\UiNumber::sar($d['loan_exposure']) }}</p>
            <div class="mt-2 h-1.5 rounded-full bg-gray-100 dark:bg-gray-700">
                <div class="h-1.5 rounded-full bg-amber-500 transition-all"
                    style="width: {{ min(100, $d['exposure_pct']) }}%"></div>
            </div>
            <p class="mt-1 text-xs text-gray-400">
                {{ __(':count active / approved loans', ['count' => $d['loan_count']]) }}</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-4 p-5 sm:grid-cols-3">
        ...
        <div class="rounded-xl {{ $cp['bg'] }} ring-1 {{ $cp['ring'] }} p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span
                    class="text-xs font-semibold text-gray-600 dark:text-gray-300">{{ __('Fund Coverage Ratio') }}</span>
            </div>
            ...
            <div class="mt-3 h-2 rounded-full bg-gray-200 dark:bg-gray-600">
                <div class="h-2 rounded-full {{ $cp['bar'] }} transition-all" style="width: {{ $d['coverage_pct'] }}%">
                </div>
            </div>
        ...
        <div class="rounded-xl {{ $ip['bg'] }} ring-1 {{ $ip['ring'] }} p-4">
            <div class="flex items-center gap-2 mb-3">
                ...
                <span
                    class="text-xs font-semibold text-gray-600 dark:text-gray-300">{{ __('Contribution Compliance') }}</span>
            </div>
            <p class="text-2xl font-bold {{ $ip['text'] }}">{{ $d['compliance_rate'] }}%</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ __(':paid of :active members paid this month', ['paid' => $d['paid_this_month'], 'active' => $d['active_count']]) }}
            </p>
            <div class="mt-3 h-2 rounded-full bg-gray-200 dark:bg-gray-600">
                <div class="h-2 rounded-full {{ $ip['bar'] }} transition-all"
        ...
        <div class="rounded-xl {{ $op['bg'] }} ring-1 {{ $op['ring'] }} p-4">
            ...
            <p class="text-2xl font-bold {{ $op['text'] }}">{{ \App\Support\UiNumber::sar($d['overdue_amount']) }}</p>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                {{ __(':count installments overdue', ['count' => $d['overdue_count']]) }}</p>
            <div class="mt-3 h-2 rounded-full bg-gray-200 dark:bg-gray-600">
                @php
