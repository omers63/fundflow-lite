...
<div class="rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
        <div class="flex items-center gap-2">
            ...
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">Contribution Trend — Last 12 Months</h3>
        </div>
        @if($s['trend'] !== 0)
        <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold
            ...
            {{ $s['trend'] >= 0 ? '▲' : '▼' }} {{ abs($s['trend']) }}% vs prev month
        </span>
        @endif
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-gray-100 dark:divide-gray-700 border-b border-gray-100 dark:border-gray-700">
        <div class="px-5 py-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">12-Month Total</p>
            <p class="mt-1 text-lg font-bold text-gray-900 dark:text-white">﷼ {{ number_format($s['total_12m'], 0) }}</p>
        </div>
        <div class="px-5 py-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">Monthly Average</p>
            <p class="mt-1 text-lg font-bold text-gray-900 dark:text-white">﷼ {{ number_format($s['avg_monthly'], 0) }}</p>
        </div>
        <div class="px-5 py-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">Best Month</p>
        ...
        <div class="px-5 py-4">
            <p class="text-xs font-medium text-gray-500 dark:text-gray-400">Last Month</p>
    ...
    <div class="flex items-center gap-4 px-6 pb-4">
        <div class="flex items-center gap-1.5">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">Total Contributions (SAR)</span>
        </div>
        <div class="flex items-center gap-1.5">
            <span class="w-3 h-0.5 bg-indigo-500 inline-block"></span>
            <span class="text-xs text-gray-500 dark:text-gray-400">Members Who Contributed</span>
        </div>
    </div>
</div>
