...
<div class="w-full max-w-none space-y-4 mb-2">
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-5">
        ...
        <div
            ...
            <div class="pl-2">
                ...
                <div class="mt-1.5 w-full rounded-full bg-gray-100 dark:bg-gray-700 h-1.5">
                    <div class="h-1.5 rounded-full {{ $compColor($d['compliance_this']) }}"
                        style="width: {{ min(100, $d['compliance_this']) }}%"></div>
                </div>
            </div>
        </div>
        ...
        <div
            class="relative overflow-hidden rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <div
                class="absolute inset-y-0 left-0 w-1 rounded-l-xl {{ $d['late_total'] > 0 ? 'bg-amber-400' : 'bg-emerald-500' }}">
            </div>
            <div class="pl-2">
                <div class="flex items-center gap-1.5 mb-2">
        ...
        <div class="px-5 py-4">
            <div class="flex items-end gap-3 h-20">
                ...
                @foreach($d['trend'] as $t)
                    ...
                    <div class="flex-1 flex flex-col items-center gap-1">
                        ...
                        <div class="w-full rounded-t-md bg-primary-500/20 dark:bg-primary-500/10 relative"
                            style="height: {{ max(4, $barH) }}%">
            ...
            <div class="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-400">
                <span class="flex items-center gap-1"><span
                        class="w-2 h-2 rounded-full bg-emerald-500 inline-block"></span>{{ __('≥90% compliant') }}</span>
                <span class="flex items-center gap-1"><span
                        class="w-2 h-2 rounded-full bg-amber-400 inline-block"></span>70–89%</span>
                <span class="flex items-center gap-1"><span
                        class="w-2 h-2 rounded-full bg-red-400 inline-block"></span>&lt;70%</span>
            </div>
        </div>
    </div>
