...
<div class="w-full max-w-none space-y-5 mb-2">
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 xl:grid-cols-7">
        ...
        {{-- Master Cash --}}
        <div
            class="relative overflow-hidden col-span-1 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:col-span-1">
        ...
        <div
            ...
            <div class="pl-2">
                <div class="flex items-center gap-1.5 mb-2">
                    <x-heroicon-o-banknotes class="w-4 h-4 text-violet-500" />
                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                        {{ __('Master Fees') }}</p>
                </div>
                <p class="text-2xl font-bold text-gray-900 dark:text-white">
                    @if(abs($d['master_fees']) >= 1000000)
                        {{ number_format($d['master_fees'] / 1000000, 2) }}M
                    @elseif(abs($d['master_fees']) >= 1000)
                        {{ number_format($d['master_fees'] / 1000, 1) }}k
                    @else
                        {{ number_format($d['master_fees'], 0) }}
                    @endif
                </p>
                <p class="mt-0.5 text-xs text-gray-400">{{ __('SAR') }} {{ number_format($d['master_fees'], 2) }}</p>
            </div>
        </div>
        ...
        {{-- Member Cash --}}
        <div
            class="relative overflow-hidden col-span-1 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="pl-2">
                ...
                <p class="mt-0.5 text-xs text-gray-400">
                    {{ __(':count accounts', ['count' => $d['member_cash_count']]) }}</p>
            </div>
        </div>
        ...
        <div
            ...
            <div class="pl-2">
                ...
                <p class="mt-0.5 text-xs text-gray-400">
                    {{ __(':count accounts', ['count' => $d['member_fund_count']]) }}</p>
            </div>
        </div>
        ...
        <div
            ...
            <div class="pl-2">
                ...
                <p class="mt-0.5 text-xs text-gray-400">{{ __(':count loan accounts', ['count' => $d['loan_count']]) }}
                </p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div
            ...
            <div
                ...
                <h4 class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    {{ __('Transaction Activity — Last 30 Days') }}</h4>
            </div>
            <div class="px-5 py-4 space-y-3">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{{ __('SAR') }}
                        {{ number_format($d['activity_credits'], 2) }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-semibold text-red-600 dark:text-red-400">{{ __('SAR') }}
                        {{ number_format($d['activity_debits'], 2) }}</span>
                </div>
                <div class="pt-2 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                    ...
                    <span
                        class="text-xs font-semibold text-gray-700 dark:text-gray-300">{{ number_format($d['activity_tx_count']) }}</span>
                </div>
                @php $netFlow = $d['activity_credits'] - $d['activity_debits']; @endphp
                <div class="flex items-center justify-between">
                    <span class="text-xs text-gray-400">{{ __('Net flow') }}</span>
                    <span
                        class="text-xs font-semibold {{ $netFlow >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400' }}">
        ...
        <div
            ...
            <div
                ...
                <h4 class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    {{ __('Balance Distribution') }}</h4>
            </div>
            @php
                $total = $d['master_cash'] + $d['master_fund'] + $d['master_fees'] + $d['member_cash_total'] + $d['member_fund_total'];
                $bars = [
                    ['label' => __('Master Cash'), 'value' => $d['master_cash'], 'color' => 'bg-sky-500'],
                    ['label' => __('Master Fund'), 'value' => $d['master_fund'], 'color' => 'bg-emerald-500'],
                    ['label' => __('Master Fees'), 'value' => $d['master_fees'], 'color' => 'bg-violet-500'],
                ...
                @foreach($bars as $bar)
                    ...
                    <div>
                        ...
                        <div class="w-full rounded-full bg-gray-100 dark:bg-gray-700 h-1.5">
                            <div class="h-1.5 rounded-full {{ $bar['color'] }} transition-all" style="width: {{ $pct }}%">
                            </div>
                        </div>
                    </div>
                @endforeach
                <p class="pt-1 text-xs text-gray-400">
                    {{ __('Total: SAR :amount', ['amount' => number_format($total, 2)]) }}</p>
            </div>
        </div>
            ...
            <div class="px-5 py-4 space-y-3">
                ...
                <div>
                    <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1">
                        ...
                        <span
                            class="{{ $covColor }} font-medium">{{ $d['coverage'] !== null ? number_format($d['coverage'], 2) . '×' : __('N/A') }}</span>
                    </div>
                    <div class="w-full rounded-full bg-gray-100 dark:bg-gray-700 h-1.5">
                        <div class="h-1.5 rounded-full {{ $covBarColor }} transition-all" style="width: {{ $covPct }}%">
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-1.5">
                            <x-heroicon-o-exclamation-triangle
                                class="w-4 h-4 {{ $d['zero_balance_count'] > 0 ? 'text-amber-500' : 'text-emerald-500' }}" />
                            <span
                                class="text-sm text-gray-600 dark:text-gray-300">{{ __('Members with zero cash') }}</span>
                        </div>
                        <span
                            class="text-sm font-semibold {{ $d['zero_balance_count'] > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                ...
                <div class="pt-2 border-t border-gray-100 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-sm font-semibold text-amber-600 dark:text-amber-400">{{ __('SAR') }}
                            {{ number_format($d['loan_outstanding'], 2) }}</span>
                    </div>
                    <div class="flex items-center justify-between mt-1">
                        <span class="text-sm text-gray-600 dark:text-gray-300">{{ __('vs. Master Fund') }}</span>
                        <span class="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{{ __('SAR') }}
                            {{ number_format($d['master_fund'], 2) }}</span>
                    </div>
                </div>
            </div>
        </div>
