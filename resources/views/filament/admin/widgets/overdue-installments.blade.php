...
@if($d['total_count'] === 0)
    ...
    <div class="rounded-2xl bg-emerald-50 dark:bg-emerald-900/10 ring-1 ring-emerald-200 dark:ring-emerald-800 p-5 flex items-center gap-3">
        ...
        <p class="text-sm font-medium text-emerald-700 dark:text-emerald-300">{{ __('No overdue installments — all repayments are on track.') }}</p>
    </div>
@else
    <div class="rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 shadow-sm overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-red-50/60 dark:bg-red-900/10">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-base font-semibold text-gray-900 dark:text-white">{{ __('Overdue Installments') }}</h3>
                <span class="inline-flex items-center rounded-full bg-red-100 dark:bg-red-900/50 px-2.5 py-0.5 text-xs font-semibold text-red-700 dark:text-red-300">
                    {{ __(':count total', ['count' => number_format($d['total_count'])]) }}
                </span>
            </div>
            <div class="flex items-center gap-4 text-sm">
                <span class="text-gray-500 dark:text-gray-400">
                    {{ __('Principal:') }} <strong class="text-red-600 dark:text-red-400">{{ \App\Support\UiNumber::sar($d['total_overdue_amount']) }}</strong>
                </span>
                @if($d['total_overdue_fees'] > 0)
                <span class="text-gray-500 dark:text-gray-400">
                    {{ __('Late fees:') }} <strong class="text-orange-600 dark:text-orange-400">{{ \App\Support\UiNumber::sar($d['total_overdue_fees']) }}</strong>
                </span>
                @endif
            </div>
        </div>
            ...
            <table class="w-full text-sm text-left">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-700 bg-gray-50/70 dark:bg-gray-800/80">
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Borrower') }}</th>
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Loan / Tier') }}</th>
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Inst #') }}</th>
                        ...
                        <th class="px-5 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 text-center">{{ __('Days Late') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/40 transition-colors">
                        ...
                        <td class="px-5 py-3 text-right font-medium text-gray-900 dark:text-white">
                            {{ \App\Support\UiNumber::sar($item['amount']) }}
                        </td>
                        <td class="px-5 py-3 text-right">
                            @if($item['late_fee'] > 0)
                            <span class="font-medium text-orange-600 dark:text-orange-400">{{ \App\Support\UiNumber::sar($item['late_fee']) }}</span>
                            @else
        ...
        <div class="px-6 py-3 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-400 dark:text-gray-500 text-center">
            {{ __('Showing 15 of :count overdue installments — open Finance → Loans to view all.', ['count' => number_format($d['total_count'])]) }}
        </div>
        @endif
    </div>
@endif
