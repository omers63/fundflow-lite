...
<div
    ...
    <div
        ...
        <div class="flex items-center gap-2">
            ...
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">
                {{ __('Loan Repayment Trend — Last 12 Months') }}
            </h3>
        </div>
    ...
    : ($s['recovery_rate'] >= 70 ? 'bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300'
        : 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300') }}">
            {{ __(':rate% recovery rate', ['rate' => $s['recovery_rate']]) }}
        </span>
    </div>
    ...
    <div
        ...
        <div class="px-5 py-4">
            ...
            <p class="text-lg font-bold text-emerald-600 dark:text-emerald-400">SAR
                {{ number_format($s['total_on_time'], 0) }}
            </p>
        </div>
        <div class="px-5 py-4">
            <div class="flex items-center gap-1.5 mb-1">
            ...
            <p class="text-lg font-bold text-amber-600 dark:text-amber-400">SAR {{ number_format($s['total_late'], 0) }}
            </p>
        </div>
        <div class="px-5 py-4">
            <div class="flex items-center gap-1.5 mb-1">
            ...
            <p
                class="text-lg font-bold {{ $s['total_overdue'] > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-400' }}">
        ...
        <div class="px-5 py-4">
            ...
            <p class="text-lg font-bold text-gray-900 dark:text-white">SAR {{ number_format($s['total_repaid'], 0) }}
            </p>
            @if($s['overdue_count'] > 0)
                <p class="text-xs text-red-500 dark:text-red-400">
                    {{ __(':count installment(s) pending', ['count' => $s['overdue_count']]) }}
                </p>
            @else
                <p class="text-xs text-emerald-500 dark:text-emerald-400">{{ __('All current') }}</p>
            @endif
        </div>
    </div>

    {{-- Chart --}}
    <div class="p-5" style="height: 280px;" wire:ignore x-data="{
            chart: null,
            init() {
                this.$nextTick(() => {
    ...
    <div class="flex flex-wrap items-center gap-4 px-6 pb-4">
        <div class="flex items-center gap-1.5">
            ...
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ __('On-time') }}</span>
        </div>
        <div class="flex items-center gap-1.5">
            <span class="w-3 h-3 rounded-sm bg-amber-400/80 inline-block"></span>
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ __('Late') }}</span>
        </div>
        <div class="flex items-center gap-1.5">
            <span class="w-3 h-3 rounded-sm bg-red-500/80 inline-block"></span>
            <span class="text-xs text-gray-500 dark:text-gray-400">{{ __('Overdue') }}</span>
        </div>
    </div>
</div>