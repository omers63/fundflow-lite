...
>
    ...
    <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
        ...
        <div class="relative overflow-hidden rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="pl-2">
                ...
                <p class="text-xl font-bold text-emerald-600 dark:text-emerald-400">{{ __('SAR') }} {{ number_format($d['credits30'], 2) }}</p>
                <p class="mt-0.5 text-xs text-gray-400">{{ __(':count entries this period', ['count' => $d['txCount30']]) }}</p>
            </div>
        </div>
        ...
        <div class="relative overflow-hidden rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="pl-2">
                ...
                <p class="text-xl font-bold text-red-600 dark:text-red-400">{{ __('SAR') }} {{ number_format($d['debits30'], 2) }}</p>
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center gap-2 px-5 py-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/70">
            ...
            <span class="ml-auto text-xs text-gray-400">{{ __('Latest :count', ['count' => $d['recent']->count()]) }}</span>
        </div>
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                    <th class="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-400">{{ __('Date') }}</th>
                    <th class="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-400">{{ __('Type') }}</th>
                    <th class="px-5 py-2.5 text-right text-xs font-semibold uppercase tracking-wide text-gray-400">{{ __('Amount') }}</th>
                    <th class="px-5 py-2.5 text-left text-xs font-semibold uppercase tracking-wide text-gray-400 hidden sm:table-cell">{{ __('Description') }}</th>
                </tr>
            </thead>
