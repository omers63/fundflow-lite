...
<div class="rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
        <div class="flex items-center gap-2">
            ...
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">Active Loan Repayment Progress</h3>
        </div>
        <span class="inline-flex items-center gap-1 rounded-full bg-primary-100 dark:bg-primary-900 px-2.5 py-1 text-xs font-medium text-primary-700 dark:text-primary-300">
            {{ $loans->count() }} active loan{{ $loans->count() > 1 ? 's' : '' }}
        </span>
    </div>
    ...
    <div class="px-6 py-5 @if(!$loop->last) border-b border-gray-100 dark:border-gray-700 @endif">
        ...
        <div class="flex flex-wrap items-start justify-between gap-3 mb-5">
            ...
            <div class="flex-shrink-0 text-center">
                ...
                <p class="mt-1 text-xs text-gray-400">Paid</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <div class="rounded-xl bg-gray-50 dark:bg-gray-700/40 p-4">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">Installments Paid</span>
                </div>
                <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1.5">
                    <span>{{ $item['paid_installments'] }} of {{ $item['total_installments'] }}</span>
                ...
                @else
                <p class="mt-2.5 text-xs text-gray-400">No pending installments</p>
                @endif
            </div>
            ...
            <div class="rounded-xl bg-gray-50 dark:bg-gray-700/40 p-4">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">Fund Portion Repaid</span>
                </div>
                <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1.5">
                    <span>SAR {{ number_format($item['repaid_to_master'], 2) }}</span>
                ...
                <p class="mt-2.5 text-xs text-gray-400">
                    @if($item['guarantor_released'])
                        <span class="text-blue-600 dark:text-blue-400 font-medium">Guarantor released</span>
                    @else
                        Guarantor holds until 100% — SAR {{ number_format($item['master_portion'], 2) }} target
                    @endif
                </p>
            </div>
            ...
            <div class="rounded-xl bg-gray-50 dark:bg-gray-700/40 p-4">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-xs font-semibold text-gray-600 dark:text-gray-300">Settlement Readiness</span>
                </div>
                <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400 mb-1.5">
                    <span>SAR {{ number_format($item['fund_balance'], 2) }}</span>
                ...
                <p class="mt-2.5 text-xs text-gray-400">
                    Need SAR {{ number_format($item['settle_required'], 2) }}
                    ({{ round($loan->settlement_threshold * 100) }}% of loan)
                </p>
            </div>
        </div>
