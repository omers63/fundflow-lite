...
<div class="rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
        <div class="flex items-center gap-2">
            ...
            <h3 class="text-base font-semibold text-gray-900 dark:text-white">Account Balances</h3>
        </div>
        <span class="text-xs text-gray-400">Total: ﷼ {{ number_format($d['cash'] + $d['fund'], 0) }}</span>
    </div>
            ...
            <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-300">Cash Balance</p>
                ...
                <p class="mt-1.5 text-xs text-gray-400">
                    {{ $d['cash_covers'] ? 'Covers' : '⚠ Insufficient for' }} {{ $d['next_due_label'] }} (﷼ {{ number_format($d['next_due'], 0) }})
                </p>
            </div>
            ...
            <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-300">Fund Balance</p>
                            <p class="text-xs text-gray-400">Savings fund</p>
                        </div>
                    </div>
                ...
                <p class="text-2xl font-bold text-gray-900 dark:text-white">﷼ {{ number_format($d['fund'], 2) }}</p>
                <div class="mt-3 h-2 rounded-full bg-gray-100 dark:bg-gray-700">
                ...
                <p class="mt-1.5 text-xs {{ $d['fund_to_go'] > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400' }}">
                    @if($d['fund_to_go'] > 0)
                        ﷼ {{ number_format($d['fund_to_go'], 0) }} more to reach loan eligibility (﷼ {{ number_format($d['min_fund'], 0) }})
                    @else
            ...
            <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-300">Max Borrowable</p>
                            <p class="text-xs text-gray-400">Loan eligibility</p>
                        </div>
                    </div>
                    <span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium
                ...
                <p class="text-2xl font-bold {{ $d['eligible'] ? 'text-sky-600 dark:text-sky-400' : 'text-gray-400' }}">
                    {{ $d['eligible'] ? '﷼ '.number_format($d['max_borrow'], 0) : '—' }}
                </p>
                <div class="mt-3 h-2 rounded-full bg-gray-100 dark:bg-gray-700">
            ...
            <div class="p-5">
                <div class="flex items-start justify-between mb-3">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="text-xs font-semibold text-gray-600 dark:text-gray-300">Monthly Allocation</p>
                            <p class="text-xs text-gray-400">Configured contribution</p>
                        </div>
                    </div>
                </div>
                <p class="text-2xl font-bold text-indigo-600 dark:text-indigo-400">﷼ {{ number_format($d['monthly_alloc'], 0) }}</p>
                <div class="mt-3 h-2 rounded-full bg-gray-100 dark:bg-gray-700">
                    <div class="h-2 rounded-full bg-indigo-400 transition-all" style="width: {{ min(100, round($d['monthly_alloc'] / 3000 * 100)) }}%"></div>
                </div>
                <p class="mt-1.5 text-xs text-gray-400">Per month · {{ round($d['monthly_alloc'] / 3000 * 100) }}% of max (﷼ 3,000)</p>
            </div>
        </div>
    </div>
</div>
