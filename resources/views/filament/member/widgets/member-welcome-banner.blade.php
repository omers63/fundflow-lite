...
<div class="space-y-3">
    ...
    <div class="relative overflow-hidden rounded-2xl shadow-xl">
        ...
        <div
            class="pointer-events-none absolute -top-20 -right-20 h-64 w-64 rounded-full bg-emerald-400 opacity-20 blur-3xl">
        </div>
        <div
            class="pointer-events-none absolute -bottom-12 -left-12 h-48 w-48 rounded-full bg-teal-300 opacity-20 blur-3xl">
        </div>
                    ...
                    @if($d['hasMember'])
                        <p class="mt-1.5 text-sm text-emerald-200">
                            ...
                            @if($d['memberStatus'] !== 'active')
                                &nbsp;·&nbsp;<span
                                    class="text-amber-300 font-semibold">{{ __(ucfirst(str_replace('_', ' ', $d['memberStatus']))) }}</span>
                            @endif
                        </p>
                        {{-- Status badges --}}
                        <div class="mt-2 flex flex-wrap gap-2">
                            @if($d['overdueCount'] > 0)
                                <span
                                    class="inline-flex items-center gap-1.5 rounded-full bg-red-500/30 ring-1 ring-red-400/40 px-3 py-1 text-xs font-semibold text-red-200">
                            ...
                            @elseif($d['paidThisMonth'])
                                <span
                                    class="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/30 ring-1 ring-emerald-400/40 px-3 py-1 text-xs font-semibold text-emerald-200">
                            ...
                            @if($d['pendingLoan'])
                                <span
                                    class="inline-flex items-center gap-1.5 rounded-full bg-blue-500/30 ring-1 ring-blue-400/40 px-3 py-1 text-xs font-semibold text-blue-200">
                ...
                @if($d['hasMember'] && $d['nextPayment'])
                    <div class="flex-shrink-0">
                        <div class="rounded-2xl bg-white/10 ring-1 ring-white/20 px-5 py-4 text-center min-w-[140px]">
                            <p class="text-xs font-medium text-emerald-300 uppercase tracking-wide">{{ __('Next Payment') }}
                            </p>
                            <p class="mt-1 text-2xl font-extrabold text-white">{{ __('SAR') }}
                                {{ number_format($d['nextPayment']['amount'], 0) }}</p>
                            <p class="mt-0.5 text-xs text-emerald-200">{{ $d['nextPayment']['label'] }}</p>
                            <span
                                class="mt-2 inline-block rounded-full {{ $d['nextPayment']['type'] === 'installment' ? 'bg-indigo-500/30 text-indigo-200' : 'bg-emerald-500/30 text-emerald-200' }} px-2 py-0.5 text-xs">
            ...
            @if($d['hasMember'])
                ...
                <div class="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <div class="rounded-xl bg-white/8 ring-1 ring-white/15 px-4 py-3 backdrop-blur-sm">
                        <p class="text-xs font-medium text-emerald-300 uppercase tracking-wider">{{ __('Cash Balance') }}
                        </p>
                        <p class="mt-1 text-xl font-bold text-white">{{ __('SAR') }} {{ number_format($d['cash'], 0) }}</p>
                    </div>
                    <div class="rounded-xl bg-white/8 ring-1 ring-white/15 px-4 py-3 backdrop-blur-sm">
                        <p class="text-xs font-medium text-emerald-300 uppercase tracking-wider">{{ __('Fund Balance') }}
                        </p>
                    ...
                    <div class="rounded-xl bg-white/8 ring-1 ring-white/15 px-4 py-3 backdrop-blur-sm">
                        ...
                        @if($d['activeLoan'])
                            <p class="mt-1 text-xl font-bold text-white">{{ __('SAR') }}
                                {{ number_format($d['activeLoan']->amount_approved, 0) }}</p>
                    ...
                    <div class="rounded-xl bg-white/8 ring-1 ring-white/15 px-4 py-3 backdrop-blur-sm">
                        ...
                        @else
                            <p class="mt-1 text-xl font-bold text-red-300">
                                {{ __(':count Overdue', ['count' => $d['overdueCount']]) }}</p>
                        @endif
                    </div>
                </div>
            @endif
    ...
    @if($d['hasMember'])
        ...
        @if($d['overdueCount'] > 0)
            <div
                class="flex items-center justify-between gap-4 rounded-xl bg-red-50 dark:bg-red-950/30 ring-1 ring-red-200 dark:ring-red-800 px-5 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-red-600 dark:text-red-400 mt-0.5">
                            {{ __('Late fees may be accruing. Pay as soon as possible to avoid further penalties.') }}</p>
                    </div>
                </div>
        ...
        @if($d['canRepay'] && !$d['repayInsufficient'])
            <div
                class="flex items-center justify-between gap-4 rounded-xl bg-blue-50 dark:bg-blue-950/30 ring-1 ring-blue-200 dark:ring-blue-800 px-5 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-blue-800 dark:text-blue-300">
                            {{ __('Loan installment is due for this period') }}</p>
                        <p class="text-xs text-blue-600 dark:text-blue-400 mt-0.5">
                            {{ __('You can pay your installment now from your cash account.') }}</p>
                    </div>
                </div>
                <a href="{{ \App\Filament\Member\Resources\MyInstallmentsResource::getUrl('index') }}"
        ...
        @elseif($d['canRepay'] && $d['repayInsufficient'])
            <div
                class="flex items-center gap-4 rounded-xl bg-amber-50 dark:bg-amber-950/30 ring-1 ring-amber-200 dark:ring-amber-800 px-5 py-4">
                <x-heroicon-o-exclamation-triangle class="w-6 h-6 text-amber-600 dark:text-amber-400 flex-shrink-0" />
                <div>
                    <p class="text-sm font-semibold text-amber-800 dark:text-amber-300">
                        {{ __('Loan installment due — insufficient cash') }}</p>
                    <p class="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                        {{ __('Your cash balance is too low to cover this month\'s installment. Contact admin to top up your account.') }}
                    </p>
                </div>
            </div>
        @endif
        ...
        @if($d['contributionDue'] && !$d['canRepay'])
            <div
                class="flex items-center justify-between gap-4 rounded-xl bg-amber-50 dark:bg-amber-950/30 ring-1 ring-amber-200 dark:ring-amber-800 px-5 py-4">
                <div class="flex items-center gap-3">
                    <x-heroicon-o-banknotes class="w-6 h-6 text-amber-600 dark:text-amber-400 flex-shrink-0" />
                    <div>
                        <p class="text-sm font-semibold text-amber-800 dark:text-amber-300">
                            {{ __('Monthly contribution not yet recorded') }}</p>
                        <p class="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                            {{ __('Ensure your contribution is processed before the cycle deadline to avoid late fees.') }}</p>
                    </div>
                </div>
        ...
        @if($d['isEligible'] && !$d['activeLoan'] && !$d['pendingLoan'])
            <div
                class="flex items-center justify-between gap-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/30 ring-1 ring-emerald-200 dark:ring-emerald-800 px-5 py-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-emerald-800 dark:text-emerald-300">
                            {{ __('You are eligible to apply for a loan') }}</p>
                        <p class="text-xs text-emerald-600 dark:text-emerald-400 mt-0.5">
                            {{ __('Your fund balance qualifies you. Use the Loan Calculator to estimate your repayments.') }}
                        </p>
                    </div>
                </div>
