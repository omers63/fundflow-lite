...
<div class="rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 shadow-sm overflow-hidden">
    ...
    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/80">
        ...
        <div class="flex items-center gap-2">
            ...
            <span class="inline-flex items-center gap-1 rounded-full bg-red-100 dark:bg-red-900/50 px-2.5 py-1 text-xs font-semibold text-red-700 dark:text-red-300">
                <x-heroicon-o-exclamation-triangle class="w-3.5 h-3.5" /> {{ __(':count overdue', ['count' => $d['overdue_count']]) }}
            </span>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 divide-x divide-y sm:divide-y-0 divide-gray-100 dark:divide-gray-700">
        ...
        <div class="flex flex-col gap-2.5 p-5">
            ...
            <div>
                <p class="text-lg font-bold text-gray-900 dark:text-white">{{ __('SAR') }} {{ number_format($d['total_contributions'], 0) }}</p>
                <p class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ __('Total Contributions') }}</p>
                <p class="mt-1 text-xs text-gray-400">{{ __(':count payment(s)', ['count' => $d['contrib_count']]) }}</p>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-2.5 p-5">
            ...
            <div>
                ...
                @if($d['streak'] > 1)
                <p class="mt-1 text-xs text-emerald-600 dark:text-emerald-400">🔥 {{ __(':count-month streak', ['count' => $d['streak']]) }}</p>
                @elseif($d['streak'] === 1)
        ...
        <div class="flex flex-col gap-2.5 p-5 {{ $d['overdue_count'] > 0 ? 'bg-red-50/40 dark:bg-red-900/5' : '' }}">
            ...
            <div>
                ...
                <p class="text-lg font-bold {{ $d['overdue_count'] > 0 ? 'text-red-600 dark:text-red-400' : 'text-sky-600 dark:text-sky-400' }}">
                    {{ __('SAR') }} {{ number_format($d['active_loan']->amount_approved, 0) }}
                </p>
