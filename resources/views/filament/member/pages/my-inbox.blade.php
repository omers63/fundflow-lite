...
<div class="space-y-3">
    ...
    <div class="rounded-xl ring-1 {{ $myUnread > 0 ? 'ring-primary-300 dark:ring-primary-700 bg-primary-50 dark:bg-primary-900/20' : 'ring-gray-200 dark:ring-gray-700 bg-white dark:bg-gray-800' }} shadow-sm overflow-hidden transition-all">
            ...
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        @if($myUnread > 0)
                        <span class="inline-flex items-center rounded-full bg-primary-600 px-2 py-0.5 text-xs font-bold text-white">
                            {{ $myUnread }} new
                        </span>
                        @endif
                    </div>
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        ...
                        @if($allMessages->count() > 1)
                            &middot; {{ $allMessages->count() }} messages
                        @endif
                    </p>
                </div>
            </div>
        ...
        <div class="border-t border-gray-100 dark:border-gray-700 px-5 py-4 space-y-4">
            ...
            <div class="flex gap-3 {{ $isMine ? 'flex-row-reverse' : '' }}">
                <div class="flex-shrink-0 h-8 w-8 rounded-full {{ $isMine ? 'bg-primary-100 dark:bg-primary-800 text-primary-700 dark:text-primary-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400' }} flex items-center justify-center text-xs font-bold">
                    {{ strtoupper(mb_substr($msg->sender?->name ?? '?', 0, 1)) }}
                </div>
                <div class="max-w-lg">
                    <div class="rounded-xl {{ $isMine ? 'bg-primary-600 text-white rounded-tr-none' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded-tl-none' }} px-4 py-3 text-sm whitespace-pre-wrap">
                        {{ $msg->body }}
                    </div>
                    <p class="text-xs text-gray-400 mt-1 {{ $isMine ? 'text-right' : '' }}">
                        {{ $msg->created_at->format('d M Y H:i') }}
                        @if(!$isMine && $msg->read_at)
                            &middot; <x-heroicon-o-check class="w-3 h-3 inline" /> Read
                        @endif
                    </p>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 dark:border-gray-700 pt-4"
                ...
                <div class="flex gap-2 items-end">
                    ...
                    <button type="button"
                            ...
                            class="inline-flex items-center gap-1.5 rounded-lg bg-primary-600 hover:bg-primary-700 px-4 py-2 text-sm font-medium text-white transition-colors">
    ...
    <div class="rounded-xl bg-white dark:bg-gray-800 ring-1 ring-gray-200 dark:ring-gray-700 p-10 text-center">
        ...
        <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">When an administrator sends you a message, it will appear here.</p>
    </div>
    @endforelse
