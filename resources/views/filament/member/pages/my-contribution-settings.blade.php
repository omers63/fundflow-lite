...
<div class="rounded-2xl bg-gradient-to-br from-primary-600 to-primary-700 dark:from-primary-700 dark:to-primary-900 p-6 text-white shadow-lg mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <p class="text-sm font-medium text-primary-100 uppercase tracking-wide">{{ __('Monthly Contribution') }}</p>
            <p class="mt-1 text-4xl sm:text-5xl font-extrabold tracking-tight">{{ __('SAR') }} {{ number_format($current) }}</p>
    ...
    <div class="mt-5">
        <div class="flex justify-between text-xs text-primary-200 mb-1.5">
            ...
            <span class="font-medium text-white">{{ __('Your level: :progress% of max', ['progress' => $progress]) }}</span>
...
<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 mb-6">
    <div class="relative overflow-hidden rounded-xl bg-white dark:bg-gray-800 p-5 ring-1 ring-gray-200 dark:ring-gray-700 shadow-sm">
        ...
        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 pl-2">{{ __('Paid This Month') }}</p>
    ...
    <div class="relative overflow-hidden rounded-xl bg-white dark:bg-gray-800 p-5 ring-1 ring-gray-200 dark:ring-gray-700 shadow-sm">
        ...
        <p class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 pl-2">{{ __('Total Contributed') }}</p>
        <p class="mt-1 text-2xl font-bold text-gray-900 dark:text-white pl-2">{{ __('SAR') }} {{ number_format($totalEver) }}</p>
