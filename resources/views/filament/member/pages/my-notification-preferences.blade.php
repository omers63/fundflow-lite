<x-filament-panels::page>
    ...
    <div class="mb-6 rounded-xl bg-gradient-to-br from-sky-100 via-white to-indigo-50 dark:from-slate-800 dark:via-sky-950/35 dark:to-indigo-950/30 ring-1 ring-sky-200/80 dark:ring-sky-600/40 p-5 shadow-md">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-white mb-4">{{ __('Available Communication Channels') }}</h3>
    ...
    <div class="space-y-3">
        ...
        <div class="hidden lg:grid lg:grid-cols-12 gap-4 px-5 pb-1">
            <div class="col-span-4 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">{{ __('Notification Category') }}</div>
            ...
            <div class="lg:grid lg:grid-cols-12 lg:gap-4 lg:items-center p-5">
                ...
                <div class="lg:col-span-8 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    >
                        ...
                        @if (!$sysEnabled)
                            <span class="text-[10px] font-medium text-red-500 dark:text-red-400 leading-none">{{ __('Admin Off') }}</span>
                        @elseif ($forced)
                            <span class="text-[10px] font-medium text-amber-600 dark:text-amber-400 leading-none">{{ __('Required') }}</span>
                        @elseif (!$supported)
                            <span class="text-[10px] text-gray-400 dark:text-gray-500 leading-none">{{ __('N/A') }}</span>
                        @elseif ($enabled)
                            <span class="text-[10px] font-medium text-primary-600 dark:text-primary-400 leading-none">{{ __('On') }}</span>
                        @else
    ...
    <div class="mt-4 rounded-lg bg-blue-50 dark:bg-blue-950/30 ring-1 ring-blue-200 dark:ring-blue-800/40 p-4">
        <p class="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
            ...
            {!! __('Make sure your phone number is set in <a href=":url" wire:navigate class="underline">My Profile</a> to receive SMS and WhatsApp notifications.', ['url' => route('filament.member.pages.my-profile-page')]) !!}
