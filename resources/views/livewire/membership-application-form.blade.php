<div class="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8 sm:py-12 px-4">
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-3xl shadow-xl p-12 text-center">
            ...
            <h2 class="text-2xl font-bold text-slate-800 mb-3">{{ __('Application Submitted!') }}</h2>
            <p class="text-slate-500 mb-2">{{ __('Your membership application has been received and is now under review.') }}</p>
            <p class="text-slate-500 text-sm mb-8">{{ __('You will be notified via email, SMS, and WhatsApp once your application is processed.') }}</p>
        ...
        <div class="bg-white rounded-3xl shadow-xl p-10 sm:p-12 text-center">
            ...
            <h2 class="text-2xl font-bold text-slate-800 mb-3">{{ __('Applications temporarily closed') }}</h2>
            <p class="text-slate-600 max-w-md mx-auto mb-2">
                {{ __('We are not accepting new membership applications right now because our review queue is full.') }}
            </p>
            <p class="text-slate-500 text-sm max-w-md mx-auto mb-8">
                {{ __('If you already applied, you can check your status below. Otherwise, please try again later.') }}
            </p>
            <div class="flex flex-col sm:flex-row gap-3 justify-center">
                <a href="{{ route('application.status') }}" class="bg-blue-600 text-white font-semibold px-6 py-3 rounded-xl hover:bg-blue-700 transition-colors">
                    {{ __('Check application status') }}
        ...
        <div class="mb-8 text-center">
            <h1 class="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">
                {{ $onBehalfMode ? __('Apply for a Dependent') : __('Apply for Membership') }}
            </h1>
            <p class="text-slate-500">
                {{ $onBehalfMode ? __('You are applying on behalf of a dependent under your household account.') : __('Complete the form below to join the FundFlow family fund.') }}
            </p>
            <a
        ...
        <div class="mb-8">
            <div class="overflow-x-auto pb-2">
                <div class="relative min-w-[34rem] sm:min-w-0 flex items-center justify-between">
                    ...
                    @php
                        ...
                        __('Personal Info') => __('Info'),
                        __('Identity') => __('Identity'),
                        __('Employment') => __('Work'),
                        __('Document') => __('Doc'),
                        __('Membership fee') => __('Fee'),
                            default => $label,
                        };
                    @endphp
                    <div class="flex min-w-[6.25rem] sm:min-w-[6rem] flex-col items-center relative z-10">
                        ...
                        <span class="mt-2 text-center text-[10px] sm:text-xs font-medium leading-tight {{ $currentStep === $i + 1 ? 'text-blue-600' : 'text-slate-400' }}">
        ...
        <div class="bg-white rounded-3xl shadow-xl overflow-hidden">
            <div class="bg-gradient-to-r from-blue-600 to-indigo-600 p-6">
                @php
                    ...
                    $stepTitle = match ($kind) {
                        'personal' => __('Personal Information'),
                        'payment' => __('Membership fee'),
                        'identity' => __('Identity & Address'),
                        'employment' => __('Employment & Next of Kin'),
                        'document' => __('Application Document'),
                        default => __('Application'),
                    };
                @endphp
                <h2 class="text-white font-bold text-xl">{{ __('Step :step: :title', ['step' => $currentStep, 'title' => $stepTitle]) }}</h2>
            </div>
            ...
            <div class="p-5 sm:p-8">
                ...
                <div class="space-y-5">
                    <div>
                        <p class="text-sm font-semibold text-slate-800 mb-3">{{ __('Application type') }} *</p>
                        <p class="text-xs text-slate-500 mb-4 leading-relaxed">{{ __('Choose the option that matches your situation. The application fee (if any) depends on this choice — you will confirm the transfer on the') }} <strong class="font-semibold text-slate-700">{{ __('last step before you submit') }}</strong>.</p>
                        <div class="grid sm:grid-cols-3 gap-3">
                            @foreach([
                                'new' => ['title' => __('New'), 'desc' => __('First-time membership with the fund.'), 'icon' => 'M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z'],
                                'resume' => ['title' => __('Resume'), 'desc' => __('Returning after a break; reinstatement.'), 'icon' => 'M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15'],
                                'renew' => ['title' => __('Renew'), 'desc' => __('Renewing an existing membership period.'), 'icon' => 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z'],
                            ] as $value => $meta)
                            @php
                    ...
                    <div>
                        ...
                        <input wire:model="name" type="text" placeholder="{{ __('Ahmed Al-Saudi') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('name') border-red-400 @enderror">
                        @error('name')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Email Address') }} *</label>
                        <input wire:model="email" type="email" placeholder="{{ __('you@example.com') }}"
                            @if($onBehalfMode) readonly @endif
                            class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('email') border-red-400 @enderror {{ $onBehalfMode ? 'bg-slate-100 text-slate-600 cursor-not-allowed' : '' }}">
                        @error('email')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        @if($onBehalfMode)
                            <p class="text-xs text-slate-500 mt-1">{{ __('Dependent uses household email by default and appears in household profile selection.') }}</p>
                        @endif
                    </div>
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Password') }} *</label>
                            <input wire:model="password" type="password" placeholder="{{ __('Min. 8 characters') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('password') border-red-400 @enderror">
                            @error('password')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Confirm Password') }} *</label>
                            <input wire:model="password_confirmation" type="password" placeholder="{{ __('Repeat password') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div class="space-y-5">
                    <div class="rounded-xl border border-blue-100 bg-blue-50/60 px-4 py-3 text-sm text-slate-700">
                        <span class="font-semibold text-blue-900">{{ __('Application type:') }}</span>
                        @php
                        ...
                        @if($hasApplicationFee)
                            @if($this->currentApplicationFeeAmount() > 0)
                                <span class="text-slate-500"> — {{ __('application fee') }} {{ __('SAR') }} {{ number_format($this->currentApplicationFeeAmount(), 2) }} ({{ __('you will confirm the bank transfer on the last step before submitting') }}).</span>
                            @else
                                <span class="text-slate-500"> — {{ __('no application fee for this type.') }}</span>
                            @endif
                        @endif
                    </div>
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Gender') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Marital status') }}</label>
                    ...
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('National ID') }} *</label>
                            <input wire:model="national_id" type="text" placeholder="{{ __('1XXXXXXXXX') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('national_id') border-red-400 @enderror">
                            @error('national_id')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Date of Birth') }} *</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Full Address') }} *</label>
                        <textarea wire:model="address" rows="3" placeholder="{{ __('Street, Building, District...') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('address') border-red-400 @enderror resize-none"></textarea>
                        @error('address')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('City') }} *</label>
                        <input wire:model="city" type="text" placeholder="{{ __('Riyadh') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('city') border-red-400 @enderror">
                        @error('city')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>
                    <p class="text-xs text-slate-400 uppercase font-semibold tracking-wide">{{ __('Contact numbers') }}</p>
                    <div class="grid sm:grid-cols-1 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Mobile phone') }} *</label>
                            <input wire:model="mobile_phone" type="tel" placeholder="{{ __('+966 50 000 0000') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('mobile_phone') border-red-400 @enderror">
                            <p class="text-xs text-slate-500 mt-1">{{ __('Used for SMS and WhatsApp and saved on your account.') }}</p>
                            @error('mobile_phone')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        </div>
                    </div>
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Home phone') }}</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Work phone') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Work place') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Residency place') }}</label>
                    ...
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Bank account number') }} *</label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('IBAN') }} *</label>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Membership date') }}</label>
                ...
                <div class="space-y-5">
                    <p class="text-xs text-slate-400 uppercase font-semibold tracking-wide">{{ __('Employment (Optional)') }}</p>
                    <div class="grid sm:grid-cols-2 gap-5">
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Occupation') }}</label>
                            <input wire:model="occupation" type="text" placeholder="{{ __('Engineer') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Employer') }}</label>
                            <input wire:model="employer" type="text" placeholder="{{ __('ARAMCO') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Monthly Income (SAR)') }}</label>
                        <input wire:model="monthly_income" type="number" step="0.01" placeholder="{{ __('10000.00') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>

                    <div class="border-t border-slate-100 pt-5">
                        <p class="text-xs text-slate-400 uppercase font-semibold tracking-wide mb-4">{{ __('Next of Kin') }} *</p>
                        <div class="grid sm:grid-cols-2 gap-5">
                            <div>
                                <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Full Name') }} *</label>
                                <input wire:model="next_of_kin_name" type="text" placeholder="{{ __('Mohammed Al-Saudi') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('next_of_kin_name') border-red-400 @enderror">
                            ...
                            <div>
                                ...
                                <input wire:model="next_of_kin_phone" type="tel" placeholder="{{ __('+966 50 000 0000') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 @error('next_of_kin_phone') border-red-400 @enderror">
                ...
                <div class="space-y-6">
                    <div class="bg-slate-50 rounded-2xl p-5 border border-slate-200">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-slate-700">{{ __('Application Form Upload') }}</p>
                                <p class="text-xs text-slate-500 mt-1">{{ __('Upload a signed copy of the membership application form. Accepted formats: PDF, JPG, PNG (max 5MB). This step is optional but recommended.') }}</p>
                            </div>
                        </div>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-3">{{ __('Signed Application Form (Optional)') }}</label>
                        <label class="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-slate-300 rounded-2xl cursor-pointer hover:border-blue-400 hover:bg-blue-50/50 transition-all">
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-sm font-semibold text-blue-600">{{ __('File selected') }}</p>
                            ...
                            <div class="text-center">
                                ...
                                <p class="text-sm font-semibold text-slate-600">{{ __('Click to upload') }}</p>
                                <p class="text-xs text-slate-400 mt-1">{{ __('PDF, JPG, PNG up to 5MB') }}</p>
                            </div>
                            @endif
                        </label>
                ...
                <div class="space-y-5">
                    <div class="overflow-hidden rounded-2xl border border-indigo-100 bg-gradient-to-br from-indigo-600 via-blue-600 to-sky-600 p-6 text-white shadow-lg shadow-indigo-500/20">
                        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                            <div>
                                <p class="text-xs font-semibold uppercase tracking-wider text-white/80">{{ __('Your selection') }}</p>
                            ...
                            <div class="rounded-xl bg-white/15 px-4 py-3 text-right backdrop-blur-sm ring-1 ring-white/20">
                                <p class="text-xs text-white/80">{{ __('Amount to transfer') }}</p>
                                <p class="text-2xl font-bold tabular-nums">{{ __('SAR') }} {{ number_format($feeNow, 2) }}</p>
                            </div>
                            @endif
                        </div>
                        @if($feeNow > 0)
                        <ul class="mt-5 space-y-2 text-sm text-white/95">
                            <li class="flex gap-2">
                                ...
                                <span>{{ __('Transfer') }} <strong class="font-semibold">{{ __('SAR') }} {{ number_format($feeNow, 2) }}</strong> {{ __('to the fund bank account using the details below.') }}</span>
                            </li>
                            <li class="flex gap-2">
                                ...
                                <span>{{ __('Your application can be') }} <strong class="font-semibold">{{ __('approved only after') }}</strong> {{ __('we can match this payment to your transfer reference.') }}</span>
                            </li>
                            <li class="flex gap-2">
                                ...
                                <span>{{ __('Fees are recorded against the fund’s') }} <strong class="font-semibold">{{ __('cash account') }}</strong> {{ __('(not the pooled master fund).') }}</span>
                            </li>
                        </ul>
                        @else
                        <p class="mt-4 rounded-xl bg-white/10 px-4 py-3 text-sm text-white/95 ring-1 ring-white/15">
                            {{ __('There is') }} <strong class="font-semibold">{{ __('no application fee') }}</strong> {{ __('for this membership type. Use') }} <strong class="font-semibold">{{ __('Submit application') }}</strong> {{ __('below to finish — no bank transfer is required.') }}
                        </p>
                        @endif
                    </div>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-slate-50/80 p-5 shadow-inner">
                        <div class="mb-3 flex items-center gap-2">
                            ...
                            <div>
                                <p class="text-sm font-bold text-slate-800">{{ __('Bank transfer details') }}</p>
                                <p class="text-xs text-slate-500">{{ __('Use exactly these details when sending') }} {{ __('SAR') }} {{ number_format($feeNow, 2) }}</p>
                            </div>
                        </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">{{ __('Your transfer reference / note') }} *</label>
                        <input wire:model="membership_fee_transfer_reference" type="text" placeholder="{{ __('As shown on your bank receipt') }}" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono @error('membership_fee_transfer_reference') border-red-400 @enderror">
                        @error('membership_fee_transfer_reference')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                        <p class="text-xs text-slate-500 mt-1">{{ __('Enter the same reference you used on the transfer so we can approve your application once the payment is verified.') }}</p>
                    </div>
                    <label class="flex items-start gap-3 cursor-pointer rounded-xl border border-slate-100 bg-slate-50/50 p-4 transition hover:bg-slate-50">
                        ...
                        <span class="text-sm text-slate-700 leading-relaxed">{{ __('I confirm that I have transferred') }} <strong class="font-semibold text-slate-900">{{ __('SAR') }} {{ number_format($feeNow, 2) }}</strong> {{ __('to the fund bank account above, and I understand that my application will be reviewed after this payment can be matched.') }}</span>
                    </label>
